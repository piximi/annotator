...
export class QuickSelectionTool extends SelectionTool {
  ...
  static setup(image: ImageJS.Image) {
    const instance = new QuickSelectionTool(image);
    ...
    _.forEach(unique, (superpixel) => {
      ...
      const [colorData, colorMask] = QuickSelectionTool.colorSuperpixelMap(
        binaryImage,
        "green"
      );
